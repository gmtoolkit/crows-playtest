...
The **game content** is MCDM's, reproduced here only to make the playtest
playable on Foundry VTT. It is confined to two places precisely so that a
...
- `packs-src/` — every compendium source, extracted from the official PDFs.
- `assets/art/` — monster illustrations and the Blood Library maps, converted
  from the packet.

**`assets/tokens/` is NOT MCDM's.** Those images are generated by
`tools/make-token-art.mjs` from SVG in this repository, contain no MCDM
artwork, and are covered by the MIT licence with the rest of the code. They are
called out separately because a takedown that deleted all of `assets/` would
remove original work for no reason.
